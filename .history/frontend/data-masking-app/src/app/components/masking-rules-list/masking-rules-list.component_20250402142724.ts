...
export class MaskingRulesListComponent implements OnInit {
  ...
  processComparisonData(data: any[], rule: MaskingRule): void {
    ...
    this.comparisonData = data.map(item => ({
      ...item,
      masked: this.applyMasking(item[rule.column_name], rule.masking_type, rule.visible_chars || 0)
    }));
  }

  applyMasking(value: any, maskingType: string, visibleChars: number): string {
