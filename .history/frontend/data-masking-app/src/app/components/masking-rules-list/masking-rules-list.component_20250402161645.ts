...
export class MaskingRulesListComponent implements OnInit {
  ...
  executeScript(): void {
    ...
    // Mostrar una advertencia sobre la ejecución del script
    const message = `IMPORTANTE: Este script contiene múltiples pasos y debe personalizarse antes de ejecutarse. 
    
    Por favor, revise y modifique el script según sea necesario para su entorno específico.
      ...
      if (this.selectedRule.id) {
        this.maskingService.applyMasking(this.selectedRule.id).subscribe({
          next: (response) => {
            this.loading = false;
            if (response.success) {
              this.snackBar.open('Script preparado para su ejecución. Revise y personalice el script antes de ejecutarlo en su base de datos.', 'Cerrar', {
                duration: 7000
              });
              // Actualizar la lista de reglas
              this.loadMaskingRules();
              // No cerramos la vista previa para que el usuario pueda ver/copiar el script
              // this.closePreview();
            } else {
              this.snackBar.open('Error al preparar el script', 'Cerrar', {
                duration: 5000
              });
            }
          },
          error: (error) => {
            this.loading = false;
            this.snackBar.open('Error al preparar el script: ' + (error.message || 'Error desconocido'), 'Cerrar', {
              duration: 5000
            });
          }
        });
  ...
  copyScriptToClipboard(): void {
    ...
    this.snackBar.open('Script copiado al portapapeles', 'Cerrar', {
      duration: 3000
    });
  }

  downloadScript(): void {
    if (!this.sqlScript || !this.selectedRule) {
      return;
    }
    
    // Crear un blob con el contenido del script
    const blob = new Blob([this.sqlScript], { type: 'text/plain' });
    ...
    a.href = url;
    a.download = `mask_${this.selectedRule.table_name}_${this.selectedRule.column_name}.sql`;
    ...
    // Mostrar mensaje de confirmación
    this.snackBar.open('Script descargado correctamente', 'Cerrar', {
      duration: 3000
    });
  }
} 